...
export class RedisService {
  ...
  async incrUpgradeAmountBought(
    userId: number,
    upgradeId: number,
    amountToIncr: number,
  ) {
    return +(await this.client.hincrbyfloat(
      `${userId}:${upgradeId}`,
      "amountBought",
      amountToIncr,
    ));
  ...
  async incrClick(
    ...
    let click = +(await this.client.incrbyfloat(
      `${userId}:CLICK`,
      amountIncremented,
    ));
    let unityToIncrement = 0;
    ...
    if (unityToIncrement > 0) {
      clickUnit = +(await this.client.incrbyfloat(
        `${userId}:CLICK_UNIT`,
        unityToIncrement,
      ));
  ...
  async incrUpgrade(
    ...
    let upgradeUnit = +(await this.client.hget(
      `${userId}:${upgradeId}`,
      "amountUnit",
    ));
    ...
    let amount = +(await this.client.hincrbyfloat(
      `${userId}:${upgradeId}`,
      "amount",
      amountIncremented,
    ));
    ...
    if (unityToIncrement) {
      upgradeUnit = +(await this.client.hincrbyfloat(
        `${userId}:${upgradeId}`,
        "amountUnit",
        unityToIncrement,
      ));
  ...
  async pay(
    ...
    if (userMoneyUnit >= amount.unit) {
      ...
      if (userMoney >= valueToDecrement) {
        userMoney = +(await this.client.incrbyfloat(
          `${userId}:MONEY`,
          -valueToDecrement,
        ));
        ...
        if (unityToDecrement > 0) {
          userMoneyUnit = await this.client.decrby(
            `${userId}:MONEY_UNIT`,
            unityToDecrement,
          );
  ...
  public async updateUserData(
    ...
    const upgradesData: UpgradeRealTime[] = [];
